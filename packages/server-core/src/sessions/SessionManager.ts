...
export class SessionManager implements ISessionManager {
  ...
  async createSession(workspaceId: string, options?: import('@craft-agent/shared/protocol').CreateSessionOptions): Promise<Session> {
    ...
    if (options?.branchFromSessionId || options?.branchFromMessageId) {
      ...
      sessionLog.info('Branch validation succeeded', {
        workspaceId,
        branchFromSessionId: branch.sourceSessionId,
        branchFromMessageId: branch.sourceMessageId,
        branchContextStrategy: branch.branchContextStrategy,
        branchFromSdkSessionId: !!branch.branchFromSdkSessionId,
        copiedMessageCount: branch.branchIdx + 1,
      })
    }
  ...
  private async tryRefreshAgentRuntime(managed: ManagedSession, reason: string): Promise<void> {
    ...
    if (backendTarget.kind === 'external') return
    const backendContext = backendTarget.backendContext
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    ...
    const sigInput = {
      ...
      provider: backendContext?.provider ?? 'anthropic',
      authType: backendContext?.authType,
      resolvedModel: backendTarget.kind === 'external'
        ? (backendTarget.resolvedModel ?? managed.model ?? 'default')
        : backendTarget.backendContext.resolvedModel,
    }
    const runtimeSignature = buildBackendRuntimeSignature(sigInput)
    const restartSignature = buildRestartRequiredSignature(sigInput)
